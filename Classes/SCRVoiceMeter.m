...
#define CheckIfAudioQueueError(status, message) { if (status != noErr) SCR_LOG_ERROR(CAT, message); }

@interface SCRVoiceMeter (/* Private Methods */)
...
static void HandleInputBuffer(void *userData, AudioQueueRef audioQueue, AudioQueueBufferRef buffer,
	...
	SCRVoiceMeter *voiceMeter = (SCRVoiceMeter *)userData;
...
- (void)__initializeComponent {
	...
	if (noErr == status) {
		SCR_LOG_DEBUG(CAT, @"AudioQueue created successfully.");
	}
...
- (BOOL)__start {
	...
	if (error) {
		...
		if (error) {
			SCR_LOG_ERROR(CAT, @"Cannot set AVAudioSession active.");
		} else {
			...
			if (noErr == status) {
				...
				SCR_LOG_DEBUG(CAT, @"AudioQueue started successfully.");
				result = YES;
			}
		}
	}
